-- Migration 001: Deploy New Two-Table Architecture
-- Execute this SQL directly in BigQuery Console
-- Project: ai-workflows-459123, Dataset: ai_usage_analytics

-- 1. Create enhanced user dimension table
...
-- 4. Create Claude fact table (comprehensive schema)
...
OPTIONS (
  description = "Comprehensive Claude ecosystem usage with platform-specific metrics",
  require_partition_filter = true
);

-- 5. Platform detection will be handled in application logic during data insertion
-- Note: BigQuery doesn't support computed columns in ALTER TABLE yet

-- Deployment complete
SELECT 'New architecture deployment completed successfully' as status;