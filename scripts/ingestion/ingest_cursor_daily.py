...
def transform_usage_record(record: Dict[str, Any], activity_date: datetime) -> Dict[str, Any]:
    ...
    return {
        ...
        # daily_spend_usd will be updated separately via UPDATE query
    }
...
def load_to_bigquery(
    ...
    """Load records to BigQuery using INSERT (avoids schema mismatch issues)"""
    if not records:
    ...
    # Build INSERT statement (better schema handling than load_table_from_json)
    insert_query = """
        INSERT INTO `ai_usage_analytics.cursor_daily_metrics`
        (activity_date, user_email, user_id, is_active, total_lines_added,
         total_lines_deleted, accepted_lines_added, accepted_lines_deleted,
         total_applies, total_accepts, total_rejects, total_tabs_shown,
         total_tabs_accepted, composer_requests, chat_requests, agent_requests,
         cmdk_usages, bugbot_usages, subscription_included_reqs,
         usage_based_reqs, api_key_reqs, most_used_model,
         apply_most_used_extension, tab_most_used_extension, client_version,
         ingestion_timestamp)
        VALUES
    """
    ...
    for r in records:
        values = f"""(
            ...
            {r['total_lines_added']},
            {r['total_lines_deleted']},
            {r['accepted_lines_added']},
            {r['accepted_lines_deleted']},
            {r['total_applies']},
            {r['total_accepts']},
            {r['total_rejects']},
            {r['total_tabs_shown']},
            {r['total_tabs_accepted']},
            {r['composer_requests']},
            {r['chat_requests']},
            {r['agent_requests']},
            {r['cmdk_usages']},
            {r['bugbot_usages']},
            {r['subscription_included_reqs']},
            {r['usage_based_reqs']},
            {r['api_key_reqs']},
            '{r['most_used_model']}',
            '{r['apply_most_used_extension']}',
            '{r['tab_most_used_extension']}',
            '{r['client_version']}',
            TIMESTAMP('{r['ingestion_timestamp']}')
        )"""
    ...
    logger.info(f"Inserted {len(records)} records to {table_id}")
    return len(records)
