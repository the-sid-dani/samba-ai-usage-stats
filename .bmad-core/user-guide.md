...
### Purpose
BMAD Method integrates with Archon MCP server to provide comprehensive project tracking and task coordination alongside the traditional story-driven workflow.

### Integration Pattern
**Each BMAD agent has `mcp_tools_context` defining:**
- **archon_server**: Task management, project coordination, documentation tracking
- **serena_server**: Code navigation, symbol editing, codebase analysis

### Story-to-Task Coordination
**When Scrum Master creates stories:**
1. **Story File Created**: `docs/stories/{epic}.{story}.story.md`
2. **Archon Task Created**: Corresponding task in Archon project tracking
3. **Dual Tracking**: Stories provide dev context, Archon provides project coordination

### Developer Workflow Integration
**When Developer implements stories:**
1. **Check Archon Tasks**: Use `find_tasks()` to understand current work
2. **Update Task Status**: Use `manage_task()` to track progress (todo → doing → review → done)
3. **Update Story**: Traditional BMAD story file Dev Agent Record sections

### Benefits
- **Project Visibility**: Archon provides project-wide task tracking
- **Progress Coordination**: Multiple agents can coordinate through Archon
- **Research Integration**: Access knowledge base during story implementation
- **Unified Tracking**: Stories + tasks provide complete development audit trail
